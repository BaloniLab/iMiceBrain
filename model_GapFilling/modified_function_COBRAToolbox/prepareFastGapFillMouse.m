function [consistModel, consistMatricesSUX, BlockedRxns] = prepareFastGapFillMouse(model, listCompartments, epsilon, filename, dictionary_file, blackList)
% modified by Esraa from fastgapfill.m function
...
if ~exist('dictionary_file','var') || isempty(dictionary_file)
    ...
    dictionary_file = 'brain_draft_updated_mets_KEGG_ID.xls';
end
...
if 1
    ...
    [BlockedRxns] = identifyFastBlockedRxnsMouse(model);
    consistModel = removeRxns(model,BlockedRxns);
end

% generate SUX
MatricesSUX = generateSUXCompMouse(consistModel,dictionary, filename,blackList,listCompartments);

%postprocessing
%test consistency of MatricesSUX
if 0
    A = fastccMouse(MatricesSUX, epsilon);
    ...
    [BlockedRxnsSUX] = identifyFastBlockedRxnsMouse(MatricesSUX);
...
while m == 0
    ...
    %MatricesSUX.MatrixPart(NullRxns==1)=[]; commented out by Esraa
